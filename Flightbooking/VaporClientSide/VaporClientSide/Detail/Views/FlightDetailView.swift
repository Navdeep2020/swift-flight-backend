...
//                    })
                    ...
                    Button(action: {
                        isBooked.toggle() // Toggle the state when the button is clicked
                        DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                            isBooked.toggle()
                        }
                    }, label: {
                        HStack {
                            Image(systemName: isBooked ? "checkmark.circle.fill" : "airplane.circle.fill")
                                .font(.title3)
                                .scaleEffect(isBooked ? 1.1 : 1.0) 
                                .animation(.easeInOut(duration: 0.3), value: isBooked)
                            
                            Text(isBooked ? "Booked!" : "BookNow")
                                .font(.title2)
                                .fontWeight(.medium)
                                .scaleEffect(isBooked ? 1.1 : 1.0)
                                .animation(.easeInOut(duration: 0.3), value: isBooked)
                        ...
                        .frame(maxWidth: 200)
                        .frame(height: 50)
                        .background(isBooked ? Color.green : Color(hexStringToUIColor(hex: "#7f8c8d"))) // Change color based on state
                        .accentColor(.white)
                        .cornerRadius(8)
